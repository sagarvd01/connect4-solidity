...
contract Connect4{
    ...
    function _minimax(uint[7][6] memory board, uint depth, int alpha, int beta, bool maximizingPlayer) private view returns (uint, int){
        ...
        if(depth == 0 || isTerminal){
            if(isTerminal){
                ...
                return (0, _scorePosition(board, AI_PIECE));
            }
        }
        ...
        if(maximizingPlayer){
            ...
            for(uint i = 0; i < validLocs.length; i++){
                ...
                (uint cl, int newScore) = _minimax(boardCopy, depth - 1, alpha, beta, false);
                if(newScore > value){
                    value = newScore;
                    column = col;
                }
            ...
            for(uint i = 0; i < validLocs.length; i++){
                ...
                (uint cl, int newScore) = _minimax(boardCopy, depth - 1, alpha, beta, true);
    ...
    function _aiMove(address player) private{
        ...
        (uint col, int minimax_score) = _minimax(bd_copy, 5, -MATH_INF, MATH_INF, true);
