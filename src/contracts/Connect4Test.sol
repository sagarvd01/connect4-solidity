...
contract Connect4{
    ...
    function callMinimax(uint8 depth, int alpha, int beta, bool maxPlayer) public returns (uint, int){
        return minimax(gameState[msg.sender].boardState, depth, alpha, beta, maxPlayer);
    }
    function minimax(uint[7][6] memory board, uint8 depth, int alpha, int beta, bool maximizingPlayer) public returns (uint, int){
        uint8 dpth = depth;
        uint[7][6] memory bd_copy = board;
        int alph = alpha;
        int bta = beta;
        ...
        if(maximizingPlayer){
            ...
            for(uint i = 0; i < validLocs.length; i++){
                ...
                uint[7][6] memory boardCopy = _copyBoard(bd_copy);
    ...
    function _aiMove(address player) private{
        ...
        (uint col,) = _minimax(bd_copy, 5, -MATH_INF, MATH_INF, true);
