...
contract Connect4{
    ...
    event ExecutedSoFar(string word);
    constructor(){
    ...
    function evaluateBoard(uint256[7][6] memory board) private pure returns (int256 score) {
    ...
    function columnFull(uint256[7][6] memory board, uint256 column) private pure returns (bool) {
        return board[column][5] != 0;
    }
    function makeMove(uint256[7][6] memory board, uint256 column, bool isMaximizingPlayer) private view returns (uint256[7][6] memory) {
